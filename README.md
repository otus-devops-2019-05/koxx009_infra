...
При выполнении команды `ansible app -m command -a 'rm -rf~/reddit'` было выполнено удаление каталога `~/reddit`.
...
`ok=2    changed=0` означает что задание выполнено успешно, при этом никаких действий на удаленном сервере для этого выполнено не было.
...
В ходе задания, были созданы различные плейбуки. Было создан сценарий описывающий все действия (`reddit_app_one_play.yml`), но он оказался не удобным т.к. для указания что-нужно установить на какой сервер необходимо указывать через `--limit` к какому хосту применять условия.
...
В конечном итоге было выполнено разделение раз отдельные плейбуки `app.yml`, `db.yml` и `deploy.yml` каждый из которых выполняет свою конкретную функцию. Для их объединения был создан плейбук - `site.yml`, который содержит в себе импорт всех остальных плейбуков.
...
В ходе задания, были выполнено разделение плейбуков по окружениям. Были созданы окружения: "stage" и "prod", которые описываются в каталоге `./environments`
Была добавлена роль из "Ansible Galaxy" - "jdauphant.nginx", которая позволяет в декларативном виде описывать процесс установки и настройки Nginx на серверах.

Кроме того, был изучен Ansible Vault, служащий для шифрования файлов-параметров, благодаря чему, появилась возможность хранить чувствительные к безопасности параметры в GIT (или другой системе контроля версий). При этом параметры остаются читаемыми для самого Ansible.
Для редактирования ранее зашифрованных файлов необходимо воспользоваться командой:
`ansible-vault edit <file>`

Для расшифровывания файла:
...
В ходе задания были изучены способы взаимодействия Ansible с Vagrant с использованием провижининга.
Были переработаны таски в ролях "app" и "db". Таски были разделены из единого файла "mail.yml" на несколько в зависимости от их назначения (puma.yml, ruby.yml, install_mongo.yml и config_mongo.yml).
Кроме этого некоторые параметры были вынесены в переменные, в частности добавилась переменная "deploy_user", которая отвечает за имя пользователя, в домашнем каталоге которого хранятся файлы конфигурации.

Были рассмотрены инструменты для тестирования ролей - "Molecule" и "Testinfra".
Molecule служит для создания машин, а Testinfra для написания тестов.
В нашем случае, в файл `ansible/roles/db/molecule/default/tests/test_default.py` были добавлены следующие проверки:
1. Запущен ли сервис "mongod"
2. Наличие строки в конфиге, благодаря которой разрешено прослушивание порта с любого IP
3. Наличие корректного порта прослушивания (27017) 
